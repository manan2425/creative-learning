...
function render(){
  ...
  document.querySelectorAll("[data-product]").forEach(b=>b.addEventListener("click",(e)=>{e.stopPropagation();openProduct(b.dataset.product,b.dataset.mediaKind||"components",b.dataset.mediaKey||"");}));
  document.querySelectorAll("[data-add]").forEach(b=>b.addEventListener("click",(e)=>{e.stopPropagation();addToCart(b.dataset.add);}));
  refreshRevealTargets();
}
function productSku(p){return String(p.sku||`CL-${p.id}`).toUpperCase();}
function card(p){const media=componentMedia(p),imgs=media.images||productImages(p);return `<article class="product-card reveal" data-product="${p.id}" style="cursor:pointer"><div class="product-body"><div class="catalogue-media">${mediaGallery(media,imgs[0]||'',p.name)}</div><span class="tag">${escapeHtml(p.category)}</span><h3 data-product="${p.id}" style="cursor:pointer">${escapeHtml(p.name)}</h3><p>${escapeHtml(p.description)}</p><div class="sku-line">SKU: ${escapeHtml(productSku(p))}</div><div class="product-meta"><span class="price">${escapeHtml(p.price)}</span><span>Ready to build</span></div><div class="card-actions"><button type="button" data-product="${p.id}">Specs</button><button type="button" class="add" data-add="${p.id}">Add to cart</button></div></div></article>`;}
function kitCard(p){const media=kitMedia(p),imgs=media.images||productImages(p);return `<article class="kit-card reveal" data-product="${p.id}" data-media-kind="kits" style="cursor:pointer"><div class="kit-body"><div class="catalogue-media">${mediaGallery(media,imgs[0]||'',p.name)}</div><span class="tag">Learning Kit</span><div class="sku-line">SKU: ${escapeHtml(productSku(p))}</div><h3 data-product="${p.id}" data-media-kind="kits" style="cursor:pointer">${escapeHtml(p.name)}</h3><p>${escapeHtml(p.description)}</p><button type="button" data-product="${p.id}" data-media-kind="kits">Kit Specs</button></div></article>`;}
